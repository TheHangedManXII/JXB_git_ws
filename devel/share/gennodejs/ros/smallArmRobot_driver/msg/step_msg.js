// Auto-generated. Do not edit!

// (in-package smallArmRobot_driver.msg)


"use strict";

const _serializer = _ros_msg_utils.Serialize;
const _arraySerializer = _serializer.Array;
const _deserializer = _ros_msg_utils.Deserialize;
const _arrayDeserializer = _deserializer.Array;
const _finder = _ros_msg_utils.Find;
const _getByteLength = _ros_msg_utils.getByteLength;

//-----------------------------------------------------------

class step_msg {
  constructor(initObj={}) {
    if (initObj === null) {
      // initObj === null is a special case for deserialization where we don't initialize fields
      this.Steps = null;
    }
    else {
      if (initObj.hasOwnProperty('Steps')) {
        this.Steps = initObj.Steps
      }
      else {
        this.Steps = [];
      }
    }
  }

  static serialize(obj, buffer, bufferOffset) {
    // Serializes a message object of type step_msg
    // Serialize message field [Steps]
    bufferOffset = _arraySerializer.int16(obj.Steps, buffer, bufferOffset, null);
    return bufferOffset;
  }

  static deserialize(buffer, bufferOffset=[0]) {
    //deserializes a message object of type step_msg
    let len;
    let data = new step_msg(null);
    // Deserialize message field [Steps]
    data.Steps = _arrayDeserializer.int16(buffer, bufferOffset, null)
    return data;
  }

  static getMessageSize(object) {
    let length = 0;
    length += 2 * object.Steps.length;
    return length + 4;
  }

  static datatype() {
    // Returns string type for a message object
    return 'smallArmRobot_driver/step_msg';
  }

  static md5sum() {
    //Returns md5sum for a message object
    return 'f848ce10d6e93faa8d4adf474668aef3';
  }

  static messageDefinition() {
    // Returns full string definition for message
    return `
    int16[] Steps
    
    
    `;
  }

  static Resolve(msg) {
    // deep-construct a valid message object instance of whatever was passed in
    if (typeof msg !== 'object' || msg === null) {
      msg = {};
    }
    const resolved = new step_msg(null);
    if (msg.Steps !== undefined) {
      resolved.Steps = msg.Steps;
    }
    else {
      resolved.Steps = []
    }

    return resolved;
    }
};

module.exports = step_msg;
